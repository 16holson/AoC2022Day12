...
def shortestPath(grid, startEnd):
    ...
    # Print the distance of the end node
        ...
        return dist[startEnd[1][0]][startEnd[1][1]]
    pass
...
def main():
    ...
    lowests = findLowestElevations(grid)
    dists = []
    for low in lowests:
        startEnd = list(startEnd)
        startEnd[0] = list(startEnd[0])
        low = list(low)
        startEnd[0][0] = low[0]
        startEnd[0][1] = low[1]
        startEnd[0] = tuple(startEnd[0])
        startEnd = tuple(startEnd)
        dists.append(shortestPath(grid, startEnd))
    print(dists)
    dists.sort()
    print(dists[0])
    file.close()
